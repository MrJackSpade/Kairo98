...
Measured on the Retroid Android device on 2026-09-24 with the same Night Slave HDI and native `-O2` build. Each interval covers 600 emulated frames. The comparison build retained the same temporary profiler, with only the Android interpreter signal-mask change between runs. The profiler has since been removed from source.
...
The jump buffer handles internal emulated exception and panic control flow. That path does not change the host thread's signal mask, so Android can omit the mask save. Other platforms keep the existing behavior. This changes host bookkeeping only; it does not change guest CPU clocks, event scheduling, audio samples, or the frame pacing policy.
