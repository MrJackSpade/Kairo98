...
The final debug APK tested here has SHA-256 `6fe08d5b653e78a8ac181af758f24489b54f8a617e9511799a01f49da4cfcda9`. It was built from these source changes plus base commit `0e2c4dd`, using `:app:assembleDebug --offline`. On the Retroid Pocket Classic (Android 14 / API 34), it reached the DOS prompt, accepted physical keys, emitted nonzero PCM buffers, survived repeated lifecycle commands, and closed a read-only HDI cleanly. [Compatibility record and screenshot](compatibility.md). The Windows 21/W reference comparison and full game execution remain pending.