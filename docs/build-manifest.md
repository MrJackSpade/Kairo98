...
Gradle 8.13, Android Gradle Plugin 8.13.2, Kotlin 2.2.20, JDK 17, Android SDK API 36, CMake 3.22.1, NDK 28.2.13676358. Release package ID `com.loxifi.kairo98`; minimum API 26; `arm64-v8a` only. The GitHub `withoutImagesRelease` APK and Google Play AAB use the same variant from the same revision and contain no `art/` catalog assets. The additional GitHub `withImagesRelease` APK uses that revision and adds embedded catalog artwork. All three contain the same emulator, JSON catalog, third-party notices, privacy policy, and Kairo98 launcher icon. There are no app-level Maven runtime dependencies. C++ is statically linked into `libkairo98.so`.
