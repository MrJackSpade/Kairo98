...
The generated Spleen 8x16 ASCII glyph table derives from pinned commit `57f9219328c9f5873085320fe8bc8f7dd34b8791`. The bundled `app/src/main/assets/font/kairo98-font.bmp` is generated by `tools/generate_font_bmp.py` from that table, the public-domain Shinonome 0.9.11p1 16-dot BDFs in `third_party/shinonome/`, and 21/W's built-in BSD font data; it contains no NEC font ROM data. The app UI's pixel-font asset `app/src/main/assets/ui/spleen-8x16-ascii.bin` holds the same glyphs, copied from that table by `tools/generate_ui_pixel_font.py`, and is covered by the same Spleen notice. The binary notices include the relevant 21/W base, IA-32, FPU, SIMD, and LIO texts, plus the full [ymfm BSD-3-Clause license](https://github.com/aaronsgiles/ymfm/blob/main/LICENSE), [Spleen BSD-2-Clause license](https://github.com/fcambus/spleen/blob/master/LICENSE), and Shinonome public-domain notice. The C++ runtime is statically linked from Android NDK 28.2.13676358, so the pinned NDK LLVM toolchain `NOTICE` is included as well; it covers LLVM exception and legacy libc++/libc++abi terms. `tools/generate_third_party_notices.ps1` assembles `app/src/main/assets/THIRD_PARTY_NOTICES.txt` from those pinned files. Both APKs and AABs include the same notice asset, accessible in About → Licenses. Neither Neko Project 21/W nor ymfm is used as the app's brand.
