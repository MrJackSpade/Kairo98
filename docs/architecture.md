...
`InputModeDecider` alone resolves Auto. It starts in Keyboard; a current blocking BIOS keyboard wait or repeated keyboard polling selects Keyboard. Four or more bus mouse reads in a 250 ms observation window select Mouse only after 750 ms without keyboard activity. Mouse stays selected through idle periods; keyboard activity switches it back. Forced Keyboard and Mouse settings bypass the Auto result. The per-game catalog `input.mode` overrides the global default; absent metadata uses Auto. A screen tap in Keyboard opens the panel without sending a guest click. While the keyboard is shown on a second display, it is always available, so a main-screen touch is always Mouse regardless of the setting. Mouse touch uses relative touchpad gestures: dragging moves the guest cursor, tapping clicks at its current position, and holding before dragging holds the left button. A 700 ms click pulse lets slow guest polling observe it. Arbitrary guest software stores its own cursor coordinates and may apply acceleration, so touchpad mode does not assume a guest cursor position. The optional Direct tap touch mouse setting maps the touch to a 640x400 guest pixel from the view's own coordinates, which already reflect scaling and any crop, then warps there: `android_host/mousemng.c` first moves the cursor 768 by 480 counts toward the top-left, where guest software clamps it, and then by the target in counts, before the click or hold is sent. The bus mouse latch keeps one signed byte per axis and drops the rest, so warp and movement counts are released only as the guest latches earlier ones. Direct tap lands on the touched point in software that clamps its cursor at the screen's top-left and moves one pixel per count without acceleration; other software lands offset or scaled. This is a conservative heuristic: guest software can bypass BIOS keyboard services, a resident driver can poll the mouse independently, and mixed-input games can remain Keyboard until a per-game override is set.

## Save states

Each library game has four save-state slots under the app's `files/states/<content ID>/`. A slot holds the core's own state file (`statsave.c` from the pinned snapshot, built unmodified), a copy of every mounted hard disk and floppy image with `disks.txt` mapping each copy to its working path, and a thumbnail read from the displayed surface with `PixelCopy`. The core's state records disks by path only; restoring the images with the state keeps the guest's cached file system consistent with the disk after a load, even when the guest wrote to the disk after the save. A save is written into a scratch directory and replaces the slot only when complete.

Both operations run on the emulation thread between frames through the command queue. Load first checks the state with `statsave_check` and rejects a missing or incompatible file before touching the running machine. It then closes the disks, restores the image copies, and calls `statsave_load`, which rebuilds the machine and rewrites the sound chip registers through the ymfm adapter. The adapter keeps its own copy of the YM2608 ADPCM RAM, so `kairo_ymfm_reload_all_ram` refreshes it from the restored core buffer. A load that fails after disks were replaced restarts the game from its disks. The in-game menu confirms before a load, an overwrite, a restart, or an exit, because none of them can be undone.
