...
Gradle 8.13, Android Gradle Plugin 8.13.2, Kotlin 2.2.20, JDK 17, Android SDK API 36, CMake 3.22.1, NDK 28.2.13676358. Package ID `com.loxifi.kairo98`; minimum API 26; `arm64-v8a` only. Build both debug APKs and both release AABs from the same revision. All contain the same emulator, JSON catalog, third-party notices, and Kairo98 launcher icon; only `withImages` contains `art/` catalog assets. There are no app-level Maven runtime dependencies. C++ is statically linked into `libkairo98.so`.
