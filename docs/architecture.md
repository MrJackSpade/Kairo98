...
When Android exposes a second logical display, `SecondaryKeyboardDisplay` presents an app-owned surface there as soon as the activity is visible. Its idle color follows the main screen: setup uses its dark background, while the library and emulator use black. During play it shows the same keyboard on that surface. It prefers displays marked for presentations and accepts another valid non-primary display on devices that omit that flag. Touch keys on either screen use the same scan-code router. The keyboard hides and releases held keys when the game menu opens; the presentation closes when the app backgrounds or the display disconnects. The main-screen flyout remains the fallback. On RG DS, which reports the lower panel as the default display, startup requests the other display for the game so the keyboard occupies the lower panel. The request is attempted once; if Android denies it, launch continues on the current display.
