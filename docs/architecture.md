...
When Android exposes a second logical display, `SecondaryKeyboardDisplay` presents the same keyboard there for the duration of play. It prefers displays marked for presentations and accepts another valid non-primary display on devices that omit that flag. Touch keys on either screen use the same scan-code router. The secondary presentation closes and releases held keys when the game menu opens, the app backgrounds, or the display disconnects; the main-screen flyout remains the fallback. On RG DS, which reports the lower panel as the default display, startup requests the other display for the game so the keyboard occupies the lower panel. The request is attempted once; if Android denies it, launch continues on the current display.
