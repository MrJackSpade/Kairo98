...
The `withoutImages` build shows **Download missing images** in the library's right menu. It fetches artwork only for games in the user's library, keeps 360-pixel thumbnails in private app storage, and resumes by skipping files already saved. The `withImages` build uses its packaged artwork and does not show that action.
